...
static int    incontent(const char*, const char*);  /*mine*/
static void   openwith(const char*);  /*mine*/
static const char   *ed;  /*mine*/
static char*  (*bstrstr)(const char *, const char *) = strstr;
...
/* Mine */ 
int
incontent(const char *fname, const char *str) {
	FILE *fp;
	char temp[1024];
	char *token;
...
match(int sub) {
	...
	for(item = sub ? matches : items; item && item->text; item = lnext) {
		...
		else if(content && incontent(item->text, text))
			appenditem(item, &lsubstr, &substrend);
	}
	matches = lexact;
	matchend = exactend;
